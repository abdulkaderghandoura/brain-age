...
def get_filtered_eeg(file_path, filters):
    ...
    for filter_type, freq in filters.items():
        if filter_type == 'lpf':
            ...
            filtered_eeg = raw.filter(l_freq=None, h_freq=freq, fir_design='firwin')
        elif filter_type == 'hpf':
            # Apply a high-pass filter to the raw data
            filtered_eeg = raw.filter(l_freq=freq, h_freq=None, fir_design='firwin')
        elif filter_type == 'nf':
            # Apply a notch filter to the raw data
            filtered_eeg = raw.notch_filter(freqs=freq, notch_widths=0.5, method='spectrum_fit')
    return filtered_eeg

...
def get_hbn_formatter(datasets_dirs):
    montage_bap = mne.channels.make_standard_montage("standard_1020")
    custom_montage_path = os.path.join(datasets_dirs, 'hbn', 'raw', 'GSN_HydroCel_129.tsv')
...
def preprocess_file(file_path, dataset_name, datasets_path=None, new_file_path=None, sfreq=None, references=[], filters={}):    
    eeg_obj = None
    if dataset_name == 'bap':
    ...
    if new_file_path is not None:
        # Save the filtered EEG data as a numpy file
...
def preprocessed_data(datasets_path, sfreq, overwrite_csv, references, dataset_names, filters):
    ...
    with multiprocessing.Pool(processes=num_processes) as pool:
        for dataset_name in dataset_names:
            ...
            output_data_path = os.path.join(output_data_dirs)
                ...
                if dataset_name == 'hbn' or (dataset_name == 'bap' and 'preprocessed' in cur_dirs):
                    for file_name in tqdm(file_names):
                        ...
                        if dataset_name == 'hbn' or (dataset_name == 'bap' and os.path.splitext(file_name)[1] == ".vhdr"):
                            ...
                            new_file_path = os.path.join(new_dir_path, os.path.splitext(file_name)[0] + '.npy')
                            pool.apply_async(preprocess_file, args=(file_path, dataset_name, datasets_path, new_file_path, sfreq, references, filters), callback=callback)
